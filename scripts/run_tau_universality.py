#!/usr/bin/env python3
"""
P2 — Universality of the exponent -1/2 across different lattice parameters τ.
...
def find_seed_via_tau_continuation(target_tau_imag, s1, k_val=3, n_steps=40):
    ...
    data_path = PROJECT / 'data' / 'full_series_k3_1000.json'
    with open(data_path) as f:
        seeds = {int(e['k']): (float(e['delta']), float(e['s2'])) for e in json.load(f)}
...
def find_seed_from_known(tau_imag, s1, k_val):
    ...
    if abs(tau_imag - 0.3205128205) < 1e-6:
        data_path = PROJECT / 'results' / 'phase15_asymptotic' / 'full_series_k3_1000.json'
        if data_path.exists():
            with open(data_path) as f:
                seeds = {int(e['k']): (float(e['delta']), float(e['s2'])) for e in json.load(f)}
            if k_val in seeds:
